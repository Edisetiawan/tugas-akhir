...
$result=mysql_query("select * from pinjam inner join denda on pinjam.pinjam_id=denda.pinjam_id where pinjam.pinjam_status='2'") or die(mysql_error());
...
{    
    $nomor=$no++;
	$pinjam = $row["anggota_nis"];    //1
    $kembali = $row["tgl_kembali"];   //2
    $nis = "Rp. ".number_format($row["bayar"],0,',','.');   //3
...
$pdf->Ln();
$pdf->Cell(80);
$pdf->Cell(30,10,'LAPORAN DATA PENGEMBALIAN',0,0,'C');
$pdf->Ln();
...
$pdf->SetX(50);                            //2     40
$pdf->Cell(50,8,'NIS',1,0,'C',1);

$pdf->SetX(100);                            //3
$pdf->Cell(50,8,'TANGGAL KEMBALI',1,0,'C',1);

$pdf->SetX(150);                            //4
$pdf->Cell(35,8,'DENDA',1,0,'C',1);
