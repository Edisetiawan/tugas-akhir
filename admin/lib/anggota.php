...
$result=mysql_query("SELECT anggota_nis,anggota_nama,kelas_nama,jurusan_keterangan,anggota_tempat,anggota_email
FROM 
    jurusan
    INNER JOIN anggota 
        ON (jurusan.jurusan_kode = anggota.jurusan_kode)
    INNER JOIN kelas 
        ON (kelas.kelas_id = anggota.kelas_id)") or die(mysql_error());
...
while($row = mysql_fetch_array($result))
{
    ...
    $kelas = $row["kelas_nama"];  //3
    $jurusan = $row["jurusan_keterangan"];  //4
...
$pdf->SetX(140);                           //5
$pdf->Cell(30,8,'Angkatan',1,0,'C',1);
